...
export async function POST(request: NextRequest) {
  try {
    ...
    const { nickname, message } = validationResult.data;
        ...
        await prisma.$transaction([
          ...
          prisma.notification.create({
            data: {
              ...
              title: '친구 요청 수락됨',
              content: `${(await prisma.user.findUnique({ where: { id: decoded.userId } }))?.nickname}님이 친구 요청을 수락했습니다.`
            }
          })
        ]);
    ...
    await prisma.notification.create({
      data: {
        ...
        type: 'friend_request',
        title: '새로운 친구 요청',
        content: `${friendRequest.sender.nickname}님이 친구 요청을 보냈습니다.`
      }
    });
