...
async function createScene(req: AuthenticatedRequest) {
  try {
    ...
    const { projectId, sceneNumber, description } = createSceneSchema.parse(body);
    ...
    const scene = await prisma.scene.create({
      data: {
        projectId,
        sceneNumber,
        name: description || `씬 ${sceneNumber}`, // description을 name으로 사용
        description,
        creatorId: req.user.userId,
      },
      include: {
    ...
    await prisma.collaborationLog.create({
      data: {
        ...
        userId: req.user.userId,
        action: "create_scene",
        details: `씬 ${sceneNumber}을(를) 생성했습니다.`,
      },
    });
