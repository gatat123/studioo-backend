...
async function handleGetStudio(request: NextRequest, context: any) {
  try {
    ...
    const stats = {
      ...
      totalComments: studio.projects.reduce((sum, p) => sum + p._count.comments, 0),
...
async function handleUpdateStudio(request: NextRequest, context: any) {
  try {
    ...
    await prisma.collaborationLog.create({
      data: {
        projectId: '', // 스튜디오 레벨 액션이므로 빈 값
        userId: user.id,
        actionType: 'studio_update',
        description: `Updated studio: ${name ? 'name changed' : ''}${description !== undefined ? ', description updated' : ''}`,
        metadata: {
          studioId,
          changes: updateData
        }
