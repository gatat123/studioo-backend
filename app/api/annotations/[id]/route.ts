...
async function getAnnotationWithPermission(
  ...
  if (!annotation.image) {
    return { error: "주석과 연결된 이미지를 찾을 수 없습니다.", status: 404 };
  }

  if (!annotation.image.scene) {
...
async function getAnnotation(
  ...
  try {
    ...
    if (!annotation.image) {
      return NextResponse.json(
        { success: false, error: "주석과 연결된 이미지를 찾을 수 없습니다." },
        { status: 404 }
      );
    }
    ...
    if (!annotation.image.scene) {
      return NextResponse.json(
        { success: false, error: "주석과 연결된 씬을 찾을 수 없습니다." },
        { status: 404 }
      );
    }
...
async function updateAnnotation(
  ...
  try {
    ...
    // annotation.image는 checkAnnotationPermissions에서 이미 확인됨
    if (!annotation.image) {
      return NextResponse.json(
        { success: false, error: "주석과 연결된 이미지를 찾을 수 없습니다." },
        { status: 404 }
    ...
    const updatedAnnotation = await prisma.$transaction(async (tx) => {
      ...
      await tx.collaborationLog.create({
        data: {
          ...
          details: `주석을 수정했습니다. (변경사항: ${Object.keys(validatedData).join(', ')})`,
        },
      });
      return updated;
    });
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.collaborationLog.create({
        data: {
          ...
          details: `주석을 삭제했습니다. (위치: ${annotation.positionX}, ${annotation.positionY})`,
        },
      });
    });
    return NextResponse.json({
