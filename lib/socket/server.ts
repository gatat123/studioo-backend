  ...
  private setupChannelEventHandlers(socket: AuthenticatedSocket) {
    ...
    socket.on("send_channel_message", async (data: {
      ...
      try {
        ...
        // 채널 업데이트 시간 갱신은 불필요 (updatedAt 필드가 없음)
    ...
    socket.on("request_channel_history", async (data: {
      ...
      try {
        ...
        const messages = await prisma.channelMessage.findMany({
          where: {
            channelId: data.channelId,
            ...(data.cursor ? { sentAt: { lt: new Date(data.cursor) } } : {})
          },
          include: {
            user: {
              select: {
        ...
        // 마지막 읽은 시간 업데이트는 불필요 (lastReadAt 필드가 없음)
      ...
      try {
        ...
        await prisma.notification.create({
          data: {
            ...
            type: 'channel_invite',
            title: '채널 초대',
            message: `${socket.user.nickname}님이 ${channel?.name} 채널에 초대했습니다`
          }
        });
      } catch (error) {
