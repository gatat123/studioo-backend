...
export interface CommentReactionEventData {
  commentId: string;
  reactionType: "like" | "dislike" | "heart" | "laugh" | "angry";
  action: "add" | "remove";
  projectId: string;
}
  ...
  private async handleCommentCreate(socket: AuthenticatedSocket, data: CommentEventData): Promise<void> {
    try {
      ...
      await CollaborationService.logActivity({
        ...
        targetId: this.getTargetId(data),
        sceneId: data.sceneId,
        description: data.parentCommentId ? "답글을 작성했습니다." : "댓글을 작성했습니다.",
        metadata: {
          commentId: data.commentId,
  ...
  private async handleCommentDelete(socket: AuthenticatedSocket, data: CommentDeleteEventData): Promise<void> {
    try {
      ...
      await CollaborationService.logActivity({
        ...
        targetId: this.getTargetId(data),
        sceneId: data.sceneId,
        description: data.isHardDelete ? "댓글을 완전히 삭제했습니다." : "댓글을 삭제했습니다.",
        metadata: {
          commentId: data.commentId,
          isHardDelete: data.isHardDelete,
        },
  ...
  private async handleCommentReaction(socket: AuthenticatedSocket, data: CommentReactionEventData): Promise<void> {
    try {
      ...
      await CollaborationService.logActivity({
        projectId: data.projectId,
        userId: socket.userId,
        actionType: data.action === "add" ? "add_reaction" : "remove_reaction",
        targetType: "comment",
        targetId: data.commentId,
        description: `댓글에 ${data.reactionType} 반응을 ${data.action === "add" ? "추가" : "제거"}했습니다.`,
        metadata: {
          commentId: data.commentId,
          reactionType: data.reactionType,
        },
